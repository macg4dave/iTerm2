...
#Uptime
uptime_time="$(uptime | awk '{print $3}' | rev | cut -c 2- | rev)"

uptime_load="$(uptime | awk '{print $8,$9,$10}')"
...
echo -e "$colour_yellow"  "Load Averages"  "*&*" "User" "*&*" "System" "*&*" "Idle" "$colour_stop" >> $tmp_file
echo -e "$colour_green"  "${uptime_load}" "*&*" "${cpu_used_user}" "*&*" "${cpu_used_sys}" "*&*" "${cpu_used_idle}" "$colour_stop" >> $tmp_file
...
echo -e "$colour_yellow"  "Root" "*&*" "Volumes Size" "*&*" "Total Used" "*&*" "Total Free" "$colour_stop" >> $tmp_file
echo -e "$colour_green"  "${startup_name}" "*&*" "${startup_size}"bs "*&*" "${startup_used}"bs "*&*" "${startup_free}"bs "$colour_stop" >> $tmp_file
